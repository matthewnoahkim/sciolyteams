...
import { AdminReviewDashboard } from '@/components/tests/admin-review-dashboard'
  ...
  return (
    <div className="container mx-auto max-w-6xl space-y-8 py-8 px-4 lg:px-6">
      ...
      <div className="space-y-6">
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <InfoItem
                icon={<Users className="h-4 w-4 text-muted-foreground" />}
                label="Max Attempts"
                value={test.maxAttempts !== null ? `${test.maxAttempts} per user` : 'Unlimited'}
              />
              <InfoItem
                icon={<Users className="h-4 w-4 text-muted-foreground" />}
                label="Total Attempts"
                value={`${test._count?.attempts || 0} attempts taken`}
              />
              {test.releaseScoresAt && (
                <InfoItem
                  icon={<Clock className="h-4 w-4 text-muted-foreground" />}
                  label="Score Release"
                  value={formatDateTime(test.releaseScoresAt)}
                />
              )}
              {test.releaseScoresAt === null && (
                <InfoItem
                  icon={<Clock className="h-4 w-4 text-muted-foreground" />}
                  label="Score Release"
                  value="Immediate (after grading)"
                />
              )}
              <InfoItem
                ...
                value={
                  test.scoreReleaseVisibility === 'SCORE_ONLY'
                    ...
                    ? 'Score + missed questions'
                    : 'Score + full test copy'
                }
              />
          ...
          {test.status === 'PUBLISHED' && (
            <AdminReviewDashboard testId={test.id} teamId={params.teamId} />
          )}
      </div>
    </div>
  )
