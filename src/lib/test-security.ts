...
/**
 * Check if scores are released for a test
 * Returns true if no release date is set (immediate) or if release date has passed
 */
export function areScoresReleased(test: {
...
/**
 * Filter attempt data based on score release visibility settings
 * Used when returning attempt results to non-admin users
 */
export function filterAttemptDataByVisibility(
  attempt: {
  ...
  if (isAdmin) {
    return {
      ...
      answers: attempt.answers.map((a) => ({
        ...
        isCorrect: a.pointsAwarded !== null && a.pointsAwarded === Number(a.question.points),
      })),
    }
  }

  const scoresReleased = areScoresReleased(test)

  if (!scoresReleased) {
    // Scores not released yet
    return {
      canViewScore: false,
      score: null,
      answers: [],
  ...
  // Scores are released - apply visibility filter
  const visibility = test.scoreReleaseVisibility || 'SCORE_ONLY'

  if (visibility === 'SCORE_ONLY') {
    return {
      canViewScore: true,
      score: attempt.gradeEarned,
      answers: [],
  ...
  if (visibility === 'SCORE_AND_MISSED') {
    // Show only questions that were answered incorrectly
    const missedAnswers = attempt.answers.filter((a) => {
      const pointsAwarded = a.pointsAwarded ?? 0
      const totalPoints = Number(a.question.points)
    ...
    return {
      canViewScore: true,
      score: attempt.gradeEarned,
      answers: missedAnswers.map((a) => ({
        questionId: a.questionId,
        question: {
          type: a.question.type,
          promptMd: a.question.promptMd,
          points: a.question.points,
          // Don't show correct answers for missed questions
          options: a.question.options?.map((opt) => ({
            id: opt.id,
            label: opt.label,
            // Don't show isCorrect
          })),
        },
        ...
        isCorrect: false, // All are incorrect since we're filtering missed
      })),
    }
  }

  // SCORE_AND_FULL_COPY - show everything
  return {
    canViewScore: true,
    score: attempt.gradeEarned,
    answers: attempt.answers.map((a) => ({
